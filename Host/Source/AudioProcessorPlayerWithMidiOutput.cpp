/*
  ...
  ==============================================================================
      ...
      Modified by Xavier Hosxe (2014) to be able to send output midi event back to the preenFM.
*/
...
//==============================================================================
void AudioProcessorPlayerWithMidiOutput::setProcessor (AudioProcessor* const processorToPlay)
{
...
//==============================================================================
void AudioProcessorPlayerWithMidiOutput::audioDeviceIOCallback (const float** const inputChannelData,
...

    {
        ...
        if (processor != nullptr)
        {
            ...
            if (! processor->isSuspended())
            {
                ...
                if (!incomingMidi.isEmpty() && deviceManager->getDefaultMidiOutput()) {
                    MidiMessage midiMessage;
                    int samplePosition;
                    MidiBuffer::Iterator midiIterator(incomingMidi);
                    while (midiIterator.getNextEvent(midiMessage, samplePosition)) {
                        deviceManager->getDefaultMidiOutput()->sendMessageNow(midiMessage);
                    }
                }
                return;
            }
...
void AudioProcessorPlayerWithMidiOutput::audioDeviceAboutToStart (AudioIODevice* const device)
{
...
void AudioProcessorPlayerWithMidiOutput::audioDeviceStopped()
{
    const ScopedLock sl (lock);
...
void AudioProcessorPlayerWithMidiOutput::handleIncomingMidiMessage (MidiInput*, const MidiMessage& message)
{
